...
StemCnt = np.array([0, 0, Diameter - Offset / math.cos(alphaRad)])
StemBtm = np.array([Diameter / math.tan(alphaRad) - Offset / math.sin(alphaRad), 0, 0])
RotateCenter = StemBtm - StemCnt
...
# 後輪中心の設定
Wheel_O_R =  np.array([np.zeros(Elements), WheelBase + np.ones(Elements), Diameter*np.ones(Elements)])
...
StemVec = RotateCenter / norm_n
# print("StemVec = ", StemVec)

# 設定した回転角と回転中心に応じた回転行列を導出
R = rtnArb_Rot(deltaTheta, StemVec)
...
# print("theta = ", theta)
WheelInit_F = [Diameter * np.cos(theta), np.zeros(Elements), Diameter * np.sin(theta)] + Wheel_O
# print("type of WheelInit_F = ", type(WheelInit_F))
# print("type of Wheel_O", type(Wheel_O))
# print("WheelInit_F = ", WheelInit_F)

# 車輪の定義（後輪）
WheelInit_R = [Diameter * np.cos(theta), np.zeros(Elements), Diameter * np.sin(theta)] + Wheel_O_R

#for theta in range(0, 360):
#    DotDot_Tmp = np.array([Diameter * np.cos(theta*np.pi/180), 0, Diameter * np.sin(theta*np.pi/180)] + Front_O)
...
for theta in range(0, 3600):    #1回の遷移は0.01°．ハンドルを360°回転させる．range()の引数に指定できるのは整数intのみ．
    
    # 設定した回転角と回転中心に応じた回転行列を導出
    R = rtnArb_Rot(theta/10, StemVec)
    ...
    # print("WheelInit_F = ", WheelInit_F)
    DotDot_Tmp = np.dot(R, WheelInit_F)
    row, col = DotDot_Tmp.shape
    print("row = ", row, "col = ", col)
    
    DirectV = np.dot(R, DirectV)
    NormDirecV = np.linalg.norm(DirectV)
    DirectV = DirectV / NormDirecV
    #print("DirectV = ", DirectV, "theta = ",theta / 10)
    xfGX.append(DirectV[0])
    yfGX.append(DirectV[1])
    zfGX.append(DirectV[2])

    # 最下点を接地点として，この時のindexを算出している．
    min_index = np.argmin(DotDot_Tmp[2,:])
    # 後輪と前輪の共通接線が接地面（地面）となるので，この時のindexを算出し，前輪接地点（x, y）を算出するのが正しい．
    # →修正要
...
dAx.text(Diameter, Diameter,     r'Chaster angle = {0} degree,  Diameter = {1} '.format(alpha, Diameter))
dAx.text(Diameter, Diameter - 2, r'Offset between center of the wheel and caster = {0} '.format(Offset))
dAx.text(Diameter, Diameter - 4, r'Steering angle is {0} degree.'.format(steering))
dAx.text(Diameter, Diameter - 6, r'Thick red line shows a trajectory which touches a ground from -{0} degree to {1} degree in steering angle'.format(steering, steering), color = 'red')
...
# 回転軸の表示
# ##################################################################
# arrow(StemCnt, StemBtm, 'k')
# line= art3d.Line3D(StemCnt[0], StemCnt[1], StemCnt[2], StemBtm[0], StemBtm[1], StemBtm[2], color="k")
Point3 = [-1*(StemCnt[0]+Diameter * 1.2), StemCnt[1], StemCnt[2]+math.tan(alphaRad)*Diameter*1.2]
ax.plot([StemCnt[0], Point3[0]],[StemCnt[1], Point3[1]], [StemCnt[2], Point3[2]], color = "k")
# ax.add_line(line)
...
# 周回後の最終位置にある車輪を描画する．
# 色は自動的に指定される．
ax.scatter( xfG, yfG, zfG, s = 1, alpha = 0.3, marker = ".")
# ステム
ax.scatter( xfGX, yfGX, zfGX, s= 1, alpha=0.3, c = "k", marker=".")
...
FFig.savefig("Trail_"+d.strftime("%B%d日%A")+"Caster="+str(alpha)+"_Wheel＝"+str(Diameter)+"_Offset="+str(Offset)+".png", dpi=100,transparent = False)
plt.show()
...
d = datetime.datetime.today()
FFig.savefig("Trail"+d.strftime("%B%d日%A")+"Caster="+str(alpha)+"_Wheel＝"+str(Diameter)+"_Offset="+str(Offset)+".png", dpi=100,transparent = False)
plt.show()
'''
'''　アニメーション機能は削除する  
